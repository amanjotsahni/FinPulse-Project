...
/* Force all main-area text to white/light so sidebar wildcard doesn't bleed */
.main *,
...
.data-table th{padding:10px 16px;color:#4A6080;font-weight:700;text-transform:uppercase;
  font-size:10px;letter-spacing:0.1em;border-bottom:1px solid rgba(96,165,250,0.12);
  text-align:left;background:rgba(96,165,250,0.04)}
.data-table td{padding:11px 16px;color:#FFFFFF!important;
  border-bottom:1px solid rgba(96,165,250,0.07);vertical-align:middle}
.data-table tr:last-child td{border-bottom:none}
.data-table tr:hover td{background:rgba(96,165,250,0.04)}
.mono{font-family:'JetBrains Mono',monospace!important;color:#FFFFFF;font-size:12px}
...
/* Main-area native Streamlit controls — button, slider, selectbox */
.main .stButton button{
  background:transparent!important;border:1px solid rgba(96,165,250,0.25)!important;
  border-radius:3px!important;color:#FFFFFF!important;font-size:11px!important;
  font-weight:600!important;letter-spacing:0.07em!important;text-transform:uppercase!important;
  transition:all 0.15s!important}
.main .stButton button:hover{
  background:rgba(96,165,250,0.08)!important;border-color:#60A5FA!important}
.main .stButton button:active{background:rgba(96,165,250,0.15)!important}
/* Slider */
...
.main [data-testid="stSlider"] [data-baseweb="slider"] div[class*="Track"]{
...
.main [data-testid="stSelectbox"] [data-baseweb="select"] div{
  background:#172340!important;border-color:rgba(96,165,250,0.2)!important;color:#FFFFFF!important}
/* Plotly chart container — remove default white border Streamlit adds */
...
def load_gold_stats() -> dict:
    ...
    try:
        ...
        df_by_type = _db_query(f"""
            SELECT type, total_transactions, fraud_count, fraud_rate_pct
            FROM {s}.fraud_rate_by_type
        ...
        df_risk = _db_query(f"""
            SELECT nameOrig AS account_id, total_transactions,
                   discrepancy_count, total_amount_transacted
            FROM {s}.high_risk_accounts
            ORDER BY discrepancy_count DESC, total_amount_transacted DESC
            LIMIT 20
        """)
...
def _unpack_stats(s: dict):
    ...
    fraud_labeled  = f"{int(total_fraud):,} confirmed" if total_fraud is not None else "Gold layer"
    tx_delta       = "Gold · fraud_rate_by_type" if s.get("total_transactions") else "Databricks unavailable"
...
if screen == "Fraud Detection":
    screen_header(
        ...
        "Fraud Detection",
        "Scans every transaction for suspicious patterns using a machine learning model "
        "trained on balance discrepancies. Flags accounts where money moved but cannot "
        "be fully accounted for — the primary signal for financial fraud.",
    )
    ...
    with c2:
        st.markdown(kpi("Confirmed Fraud", fmt_millions(_total_fraud), _fraud_labeled, "red"), unsafe_allow_html=True)
    with c3:
        st.markdown(kpi("Fraud Rate", _fraud_rate, "Across all transaction types"), unsafe_allow_html=True)
    with c4:
    ...
    if _fraud_by_type:
        st.markdown('<div class="section-label">Fraud Rate by Transaction Type</div>', unsafe_allow_html=True)
        rows_html = ""
        for row in _fraud_by_type:
            ...
            rows_html += f"""
            <tr>
                <td><span class="mono">{row.get('type','—')}</span></td>
        ...
        <div class="fp-card" style="padding:0;overflow-x:auto;">
            <table class="data-table">
                <thead><tr>
                    <th>Transaction Type</th><th>Total</th>
                    <th>Fraud Count</th><th>Fraud Rate</th>
                </tr></thead>
                <tbody>{rows_html}</tbody>
            </table>
        </div>""", unsafe_allow_html=True)

    st.markdown('<div class="section-label">Detection Engine</div>', unsafe_allow_html=True)
    run_hunt()
...
# ── INVESTIGATE ───────────────────────────────────────────────────────────────
    ...
    screen_header(
        ...
        "Deep-dive into accounts flagged by the detection engine. "
        "Cross-references transaction history, balance movements, and market data "
        "to build a complete picture of each suspicious account.",
    )
    ...
    if st.session_state.agent1_results is None:
        ...
        with c1:
            st.markdown(kpi("High-Risk Accounts", _flagged_str, "Gold · high_risk_accounts"), unsafe_allow_html=True)
        with c2:
            st.markdown(kpi("Total Exposure", _total_exposure, "Discrepancy transactions"), unsafe_allow_html=True)
        with c3:
            st.markdown(kpi("LLM Backend", "Gemini 2.0 Flash", "250K context"), unsafe_allow_html=True)

        guard_message(
            "<strong>Awaiting Hunt Agent</strong> — "
            "High-risk accounts below are sourced live from the Gold layer. "
            "Run Fraud Detection to layer anomaly scores on top for full investigation."
        )
        ...
        if _top_risk:
            ...
            for row in _top_risk:
                rows_html += f"""
                <tr>
                    <td><span class="mono">{row.get('account_id','—')}</span></td>
                    <td class="mono">{int(row.get('total_transactions') or 0):,}</td>
                    <td class="mono">{int(row.get('discrepancy_count') or 0):,}</td>
            ...
            <div class="fp-card" style="padding:0;overflow-x:auto;">
                <table class="data-table">
                    <thead><tr>
                        ...
                        <th>Discrepancies</th><th>Total Transacted</th><th>Status</th>
                    </tr></thead>
                    <tbody>{rows_html}</tbody>
                </table>
            </div>""", unsafe_allow_html=True)
        ...
        pending_placeholder(
            title="Chat Interface — Pending Build",
            desc="Conversational RAG-powered investigation of flagged accounts.",
            features=[
    ...
    if st.session_state.agent3_assessment is None:
        ...
        if _top_risk:
            ...
            <div class="fp-card" style="padding:0;overflow-x:auto;">
                <table class="data-table">
                    <thead><tr>
                        <th>Account ID</th><th>Transactions</th>
                        <th>Discrepancies</th><th>Exposure</th>
        ...
        if _stock_rankings:
            ...
            for row in _stock_rankings:
                ...
                rows_html += f"""
                <tr>
                    <td><span class="mono" style="color:#60A5FA">{row.get('ticker','—')}</span></td>
